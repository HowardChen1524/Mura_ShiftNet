...
class SupUnsupDataset(BaseDataset):
    ...
    def set_sup(self):
        transform_list = [transforms.Resize([256, 256], interpolation=InterpolationMode.BILINEAR),
                          tjwei_augumentation(),
                          transforms.ToTensor()]
        self.sup_transform = transforms.Compose(transform_list)  
    ...
    def set_unsup(self): 
        ...
        self.unsup_transform = transforms.Compose(transform_list)  

    def read_sup_img(self, fp):
        img = Image.open(fp)
        return self.sup_transform(img)
    
    def read_unsup_img(self, fp):
        img = cv2.imread(fp)  
        ...
        # pil_img.save(f"{b_dir}/{fp.split('/')[-1]}.png")
        ...
        if (~self.opt.isTrain) and self.opt.isPadding: 
            ...
            # pil_img = tensor2img(A_img)
            # pil_img = enhance_img(pil_img)
            # pil_img.save(f"{fp.split('/')[-1]}_ud.png")
        ...
        return {'A': A, 'B': B, 'M': mask, 'A_paths': fp}
    

    def __getitem__(self, index):
        A_path = self.A_paths[index]
        sup_img = self.read_sup_img(A_path)
        unsup_img = self.read_unsup_img(A_path)
